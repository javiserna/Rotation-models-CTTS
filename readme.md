...
The simulation outputs provide an evolutive track of the stellar rotation (v sin(i) and/or Prot) for the specific input values [$M\_{\ast}$, $B_{\ast}$, $\chi$, $P^{in}\_{rot}$, $\dot{M}^{in}_{acc}$]. These values are particularly valuable as they can be directly compared with observational data.

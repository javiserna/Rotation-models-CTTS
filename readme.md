...
**Future Works:**
- Measurements of periods in CTTS can be tricky to obtain because of complex brightness variations generated by various mechanisms, such as the rotation of hot and cold starspots, accretion bursts (Espaillat et al. 2021), occultations by dusty disk structures (Ansdell et al. 2019), and accretion streams falling at random locations on the star (Kurosawa & Romanova 2013). As new period measurements in CTTS are available (For instance, TESS, LSST, NGRT) it is possible to replicate the last plots using evolutionary tracks of $`P_{rot}`$.

...
We also present a grid of models using a parameters space of expected values in CTTS (~$`2.2 \times 10^{6}`$ evolutive tracks in total) and present a comparison of the model's grid with observations.
...
However, measurements of periods in CTTS can be tricky to obtain because of complex brightness variations generated by various mechanisms, such as the rotation of hot and cold starspots, accretion bursts (Espaillat et al. 2021), occultations by dusty disk structures (Ansdell et al. 2019), and accretion streams falling at random locations on the star (Kurosawa & Romanova 2013). Despite these challenges, in some cases is possible to get a pure signature of the rotation in the light curve of these objects.

...
central_values=Bin1_Macc[:,1].astype(float)
Macc_=np.linspace(min(central_values),max(central_values),10)

#################################################################
# Sample random x values
num_samples = 100  # Change this to the number of samples you want
...
# Calculate the cumulative sum of bin heights
...
# Plot the CDF
cdf_interpol = interp1d(central_values,cdf,kind='linear')

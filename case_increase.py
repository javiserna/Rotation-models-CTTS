...
if tbin==0.5:
    ...
    plot_="./Plots/bin1_vsini_B_increase.png"
    plot_2="./Plots/bin1_B_case_increase.png"
    filename_2="./Results/bin1_parameters_result.txt"
    databin = np.loadtxt('./Plots/vsini_bin1.txt')

if tbin==1.5:
    cdf_values = norm.cdf(Bfield, loc=1500, scale=200)
    Bin1_Macc=np.genfromtxt('mdot_bin1_logmdot', comments='#', dtype='S')
    filename_1="./Results/bin2_vsini_result.txt"
    plot_="./Plots/bin2_vsini_B_increase.png"
    plot_2="./Plots/bin2_B_case_increase.png"
    filename_2="./Results/bin2_parameters_result.txt"
    databin = np.loadtxt('./Plots/vsini_bin2.txt')

if tbin==2.5:
    cdf_values = norm.cdf(Bfield, loc=2000, scale=200)
    Bin1_Macc=np.genfromtxt('mdot_bin1_logmdot', comments='#', dtype='S')
    filename_1="./Results/bin3_vsini_result.txt"
    plot_="./Plots/bin3_vsini_B_increase.png"
    plot_2="./Plots/bin3_B_case_increase.png"
    filename_2="./Results/bin3_parameters_result.txt"
    databin = np.loadtxt('./Plots/vsini_bin3.txt')

if tbin==8:
    cdf_values = norm.cdf(Bfield, loc=2500, scale=200)
    Bin1_Macc=np.genfromtxt('mdot_bin1_logmdot', comments='#', dtype='S')
    filename_1="./Results/bin4_vsini_result.txt"
    plot_="./Plots/bin4_vsini_B_increase.png"
    plot_2="./Plots/bin4_B_case_increase.png"
...
if __name__ == '__main__':
    ...
    num_samples = 100  #[62 68 40 30] # Change this to the number of samples you want
    ...
    for k in range(repetitions):
        ...
        #ks_statistic, ks_pvalue = ks_2samp(np.array(vhis[k]), databin)
        #print(ks_statistic, ks_pvalue)
        if k==0:
            plt.figure(7)
            plt.hist(random_values_B, bins=10, alpha=0.1, histtype='step',label="Random sample")
            plt.xlabel(r"$B_{\ast}\,\,\,(G)$")
            plt.legend()
            plt.xlim(500,3500)
            plt.savefig(plot_2,dpi=300)

        #plt.hist(np.array(vhis[k]), bins=10, alpha=0.1, histtype='step')
        #plt.xlim(0,100)

    plt.figure(1)
    plt.hist(np.array(result[0])[0], bins='auto', density=False, alpha=0.5, label='Random Sample')
    plt.xlabel(r"$P^{in}_{rot}\,\,\,(d)$")
    plt.legend()

    plt.figure(2)
    plt.hist(np.array(result[0])[2], bins=13, density=False, alpha=0.5, label='Random Sample')
    plt.xlabel(r"$B_{\ast}\,\,\,(G)$")
    plt.legend()

    plt.figure(3)
    plt.hist(np.log10(np.array(result[0])[1]), bins=5, density=False, alpha=0.5, label='Random Sample')
    plt.xlabel(r"$\log(\dot{M}_{acc})\,\,\,(\frac{M_{\ast}}{yr})$")
    plt.legend()
    #plt.hist(bin_, bins=5, cumulative=False, density=True, alpha=0.5, label='Observations')
    #plt.plot(Macc_,cdf_values_2, label='CDF')

    # Calculate histograms for each repetition
    bin_edges_ = [np.histogram(np.array(vhis[i]), bins=10)[1] for i in range(repetitions)]
    ...
    histograms = [np.histogram(np.array(vhis[i]), bins=np.linspace(min_bin_edges, max_bin_edges, bin_step)) for i in range(repetitions)]
    ...
    ks_statistic, ks_pvalue = ks_2samp(databin, median_histogram)
    plt.figure(4)
